...
1. **Signed (IP / bipolar) late-time data is inverted via the asinh transform** (Farquharson & Oldenburg 1998, #78) — negatives with amplitude above the noise-floor censor are genuine data to the misfit. The remaining caveat: the forward models IP only through the bipolar-train superposition, not an explicit chargeability model, so strongly IP-affected decays fit with a resistivity-only model and may leave a chi residual — treat a systematically underfit line over chargeable cover as a modeling limit, not noise.
...
3. **Layback/lever-arm is a STATIC correction that must be configured** (#70). Ingest shifts the GPS-antenna position to the bird using `tow.layback_m` / `tow.drop_m` from `instrument.yaml` — measure and set these before the first real flight (a missing `tow` block warns loudly; the shipped zeros are correct only for the synthetic generator). Dynamic cable swing and bird attitude are NOT corrected — they need an attitude stream (drop-in via the ingest registry) and remain future work.
